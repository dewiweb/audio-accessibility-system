...
// Double interface (production)     : bind séparés admin (régie) et public (WiFi)
...
// ─── Démarrage ────────────────────────────────────────────────────────────────
// Mode double interface : un second serveur HTTPS (publicServer) est créé sur
// l'interface WiFi public. Il sert /, /hls et /api mais bloque /admin.
let publicServer = null;

if (isDualNetwork) {
  const publicApp = express();
  // Réutilise les mêmes middlewares sécurité
  ...
  publicApp.use('/hls', (req, res, next) => {
    if (req.path.endsWith('.m3u8')) {
      res.setHeader('Cache-Control', 'no-cache, no-store, must-revalidate');
      res.setHeader('Access-Control-Allow-Origin', '*');
    } else if (req.path.endsWith('.ts')) {
  ...
  publicApp.use((err, req, res, next) => {
    ...
    res.status(status).json({ error: process.env.NODE_ENV === 'production' && status >= 500 ? 'Internal server error' : err.message });
  });
...
const shutdown = (sig) => {
  ...
  const servers = [server, ...(publicServer ? [publicServer] : [])];
